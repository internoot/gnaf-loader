...
CREATE TABLE admin_bdys.locality_bdys(
  ...
  geom geometry(Multipolygon, 7844, 2) NOT NULL
) WITH (OIDS=FALSE);
...
CREATE TEMPORARY TABLE temp_districts (
  ...
  state text NOT NULL,
  locality_class text NOT NULL,
  geom geometry(Multipolygon, 7844, 2) NULL
) WITH (OIDS=FALSE);
...
SELECT '250190776' AS locality_pid,
  ...
  WHERE ST_Intersects(ST_Transform(ST_SetSRID(ST_MakePoint(136.1757, -35.0310), 4283), 7844), geom);
...
select locality_pid,
       ...
       state,
       locality_class,
       ST_Multi((ST_Dump(ST_Split(geom, ST_Transform(ST_GeomFromText('LINESTRING(144.96691 -37.82135,144.96826 -37.81924,144.97045 -37.81911,144.97235 -37.81921,144.97345 -37.81955,144.97465 -37.82049,144.97734 -37.82321,144.97997 -37.82602,144.98154 -37.82696,144.98299 -37.82735,144.98499 -37.82766,144.9866 -37.82985)', 4283), 7844)))).geom) AS geom
  from admin_bdys.locality_bdys
...
UPDATE temp_bdys
  SET locality_pid = locality_pid || '_2',
      ...
      postcode = '3004'
  WHERE ST_Intersects(ST_Transform(ST_SetSRID(ST_MakePoint(144.9781, -37.8275), 4283), 7844), geom);
